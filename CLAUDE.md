...
## BSU Research AI — Deployment Guide

This repo is a self-hosted fork of upstream [LibreChat](https://github.com/danny-avila/LibreChat),
branded and run as **BSU Research AI**. This section is a map of *this specific deployment* — where
its runtime config, branding, and infra live — so a "change X" request can go straight to the right
file. Everything below "Project Overview" is the general upstream contributor guide and still
applies when editing source code.

### Quick reference — "I want to change..."

| Request | Go to |
|---|---|
| App name / browser tab title / header title | [.env](.env) → `APP_TITLE` (UI section, ~line 1196). Also update the static fallback in [client/index.html](client/index.html) (`<title>`, meta description) since it renders before the JS app sets the real title. |
| Favicon / logo / app icons | [client/public/assets/](client/public/assets/) — `favicon-*.png`, `apple-touch-icon-180x180.png`, `icon-192x192.png`, `maskable-icon.png`, `logo.svg`. Referenced by path from `client/index.html`. |
| Any runtime setting (ports, API keys, model providers, auth, RAG, email, feature toggles) | [.env](.env) is the live config actually loaded — **not** `.env.example` (template only, keep both in sync when adding new keys). See section map below. |
| Model providers / endpoints (OpenAI, Azure, Bedrock, Assistants, etc.) | `.env` → `Endpoints` section (~line 394 onward); or a `librechat.yaml` if one is introduced (see note below). |
| Advanced/structured config (custom endpoints, interface toggles, file limits, rate limits) | Not currently active — only [librechat.example.yaml](librechat.example.yaml) exists as a template. To activate, copy it to `librechat.yaml` at repo root and mount it (see `docker-compose.yml`) or set `CONFIG_PATH` in `.env`. |
| Docker services (api, admin panel, mongodb, meilisearch, vector db, rag api) | [docker-compose.yml](docker-compose.yml) (base) + [docker-compose.override.yml](docker-compose.override.yml) (local overrides, e.g. exposed ports). |
| Admin panel (user/role management UI) | `admin-panel` service in `docker-compose.yml` — separate prebuilt image (`librechat-admin-panel`), configured via `ADMIN_PANEL_*` env vars in `.env`. Not part of the `client/` source tree. |
| User/account management from the CLI (ban, reset password, set balance, migrations) | [config/](config/) scripts, run via root `package.json` scripts, e.g. `npm run ban-user`, `npm run reset-password`, `npm run set-balance`, `npm run list-users`. |
| Backend API behavior (legacy Express routes/services) | [api/](api/) — JS, wiring only per workspace rules below; real logic belongs in `packages/api`. |
| New/shared TypeScript backend logic | [packages/api/](packages/api/) |
| Database models/schemas | [packages/data-schemas/](packages/data-schemas/) |
| Shared types/endpoints used by both frontend and backend | [packages/data-provider/](packages/data-provider/) |
| Frontend UI (React) | [client/src/](client/src/) — components in `components/`, routes in `routes/`, state in `store/`, API hooks in `data-provider/`, strings in `locales/en/translation.json`. |
| Search (Meilisearch) setup/init | [search/](search/) — `compose.yml`, `init/`, `healthcheck.sh`. |
| Shared "Skills" available to all users (Skills feature) | [skill/](skill/) — one folder per skill with a `SKILL.md`. |
| Kubernetes/Helm deployment | [helm/](helm/) |
| End-to-end tests | [e2e/](e2e/) (Playwright) |
| Docker/env helper scripts | [utils/](utils/) — `utils/docker/`, `utils/update_env.py`. |

### Things to never hand-edit or commit

- [data-node/](data-node/) — MongoDB's actual on-disk WiredTiger data files (the running database's
  storage volume, not source code).
- [logs/](logs/) — runtime log output.
- `client/dist/` — built frontend output, regenerated by the build.
- `.env` contains live secrets/API keys — treat it as sensitive, never print its full contents or
  commit secret values elsewhere.

### `.env` section map (root [.env](.env), also mirrored in `.env.example`)

Roughly in file order: Server Configuration → Endpoints (incl. AWS Bedrock, Assistants API, Azure
Assistants API) → Search → Speech to Text & Text to Speech → Code Interpreter → RAG → User System →
Mailgun API → **UI** (app title/branding, footer, help link) → Others → Cache Headers → OpenWeather.
Grep the `#===` banner comments in the file to jump straight to a section.

### Monorepo workspaces (`package.json` → `workspaces`)

`api`, `client`, `packages/*` (`packages/api`, `packages/client`, `packages/data-provider`,
`packages/data-schemas`). Root `npm run backend` / `npm run frontend:dev` start the app; see
"Development Commands" further down for the full list.
